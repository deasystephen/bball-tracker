...
Hooplings (formerly "Basketball Tracker" — the repo, `bball-tracker` EAS slug and `com.bballtracker.mobile` bundle/package ids keep the old identifier; the URL scheme is `hooplings://` since #504, with the old `bball-tracker` scheme still registered natively and allowlisted on the backend until the dated follow-up on 2026-12-01) is a monorepo with three packages: a React Native/Expo mobile app, a Node.js/TypeScript backend, and a Next.js web app at `web/` that hosts the public `hooplings.com/invite/<token>` accept flow (deep-links into mobile via Universal Links). Real-time game tracking uses Socket.io broadcasts backed by PostgreSQL; statistics are computed by the stats service when games finish.
...
**Runtime version 1.3.0 (#504; 1.2.0 was audit #52):** `app.config.js` uses `runtimeVersion: { policy: 'appVersion' }`, so bumping `version` moves the OTA target. `1.1.0` → `1.2.0` happened when `expo-secure-store` (native) landed (build #25+; build #24 stays on the last 1.1.0 OTA). `1.2.0` → `1.3.0` happened with the URL scheme rename: `expo-linking` resolves the sign-in redirect scheme from the **OTA manifest** (`Constants.expoConfig.scheme`), while the schemes a binary answers are baked into Info.plist, so a manifest naming `hooplings` must never reach builds #25–#30, which register only the old scheme. Every `eas update` from then on reaches **1.3.0 builds only** (build #31+); #25–#30 keep the last 1.2.0 OTA (`9fc0a45d…`, already on `api.hooplings.com`). Rule for any future scheme change: bump `version`, cut the build from the branch and verify sign-in on device **before** merging (a merge with no binary on the new runtime freezes OTA delivery). Native-module additions are also dependabot-ignored (`.github/dependabot.yml`, `expo-secure-store` included) because the Expo SDK pins them.
**URL scheme (#504):** `scheme: ['hooplings', 'bball-tracker']` in `app.config.js`; sign-in passes `config/env.ts#APP_URL_SCHEME` explicitly to `Linking.createURL` (never rely on array order — the manifest is the OTA payload). expo-router strips the scheme before routing, so old `bball-tracker` deep links still open build #31+. The overlap has one backend home, `ALLOWED_REDIRECT_SCHEMES=hooplings,bball-tracker` in `infra/task-definition.json` (the code default is `hooplings` only); WorkOS Staging lists both callback URIs, Production (#24) must list only the new one. The dated follow-up (2026-12-01, after TestFlight #30 expires) drops the allowlist entry, the WorkOS URI and, with the next native build after that date, the array entry. The brand guard retires the `bball-tracker://` link form (bare slug and bundle id stay allowed) and also scans `web/app`. **A dev client built before this branch does not register `hooplings`** — after checkout, `npx expo prebuild --platform ios --clean` then `npx expo run:ios` (run:ios alone reuses the stale `ios/` project), or local sign-in bounces to Safari and Maestro `openLink` flows fail.
